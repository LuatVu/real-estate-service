...
public interface UserBalancesRepository extends JpaRepository<UserBalances, String> {
    ...
    @Query("Select ub from UserBalances ub where ub.user.userId = :userId AND ub.status = 'ACTIVE' AND ub.expiredDate > CURRENT_TIMESTAMP AND ub.balanceType = :balanceType")
    UserBalances findByUserIdAndBalanceType(@Param("userId") String userId, @Param("balanceType") BalanceType balanceType);
}
